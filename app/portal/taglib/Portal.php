...
class Portal extends TagLib
{
    ...
    protected $tags = [
        ...
        'categories_kui'       => ['attr' => 'where,order', 'close' => 1],//非必须属性item
        'articles_show_industry'         => ['attr' => 'field,where,limit,order,page,relation,returnVarName,pageVarName,categoryIds,categoryName', 'close' => 1],//非必须属性item
    ];
...
parse;
    ...
    //自定义标签  必须在protected $tags 有定义
    public function tagCategories_kui($tag, $content)
    {
        $item          = empty($tag['item']) ? 'vo' : $tag['item'];//循环变量名
        $order         = empty($tag['order']) ? '' : $tag['order'];
        $returnVarName = 'portal_categories_data';
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\portal\service\ApiService::categories_nav_kui([
    'where'   => {$where},
    'order'   => '{$order}',
]);
...
parse;
    ...
    public function tagArticles_show_industry($tag, $content)
    {
        $item          = empty($tag['item']) ? 'vo' : $tag['item'];//循环变量名
        $order         = empty($tag['order']) ? 'post.published_time DESC' : $tag['order'];
        $relation      = empty($tag['relation']) ? '' : $tag['relation'];
        ...
        $categoryName='行业资讯';
        if (!empty($tag['categoryName'])) {
            $categoryName=$tag['categoryName'];
        }
        $portalCategoryModel = new PortalCategoryModel();
        $result=$portalCategoryModel->where('name',$categoryName)->find();
        $categoryIds = 0;
        if (!empty($result)){
